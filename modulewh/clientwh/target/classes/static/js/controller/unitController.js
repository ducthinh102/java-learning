...
define(['require', 'angular'], function (require, angular) {
	app.aController(clientwh.prefix + 'unitController', ['$scope', '$mdToast', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader', clientwh.prefix + 'userService',clientwh.prefix + 'catalogService', 'params',
		function($scope, $mdToast, $state, $rootScope, $mdDialog, $http, $log, $window, $location, $filter, $translate, $translatePartialLoader, userService, catalogService, params) {
		
		if(typeof(clientwh.translate.unit) === 'undefined' || clientwh.translate.unit.indexOf($translate.use()) < 0) {
			console.log(clientwh.translate.unit);
			if(typeof(clientwh.translate.unit) === 'undefined') {
				clientwh.translate.unit = '';
			}
			clientwh.translate.unit += $translate.use() + ';';
			$translatePartialLoader.addPart(clientwh.contextPath + '/js/common/message/unit');
			$translate.refresh();
		}

		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
			console.log('clientwh_unit_title');
			$scope.title = $translate.instant('clientwh_unit_title');
		});
	...
	$scope.catalog = {};
		...
		$scope.cofirmDeleteToastForm = function(){
			$mdToast.show(
					{  	templateUrl : clientwh.contextPath + '/view/toast.html',
						hideDelay:5000,
						controller  : 'clientwhunitController',
		...
		$scope.initList = function() {
			// Get permission.
			userService.definePermissionByTarget($scope, 'unit').then(
				// success.
				function(response) {
					// List data.
		...
		$scope.showDialogDetail = function () {
	        var htmlUrlTemplate = clientwh.contextPath + '/view/unit_view.html';
	    ...
	    $scope.showDialog = function () {
	    	$mdToast.hide();
	    	var htmlUrlTemplate = clientwh.contextPath + '/view/unit_form.html';
	    	clientwh.showDialog($scope, $mdDialog, htmlUrlTemplate).then(function(evt) {
	    		console.log('closed');
	    	}, function(evt) {
	    		console.log('not closed');
