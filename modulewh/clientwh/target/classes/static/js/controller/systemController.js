...
define(['require', 'angular'], function (require, angular) {
	app.aController(clientwh.prefix + 'systemController', ['$scope', '$mdToast', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader',clientwh.prefix + 'catalogService', clientwh.prefix + 'userService', 'params',
		...
		if(typeof(clientwh.translate.system) === 'undefined' || clientwh.translate.system.indexOf($translate.use()) < 0) {
			console.log(clientwh.translate.system);
			if(typeof(clientwh.translate.system) === 'undefined') {
				clientwh.translate.system = '';
			}
			clientwh.translate.system += $translate.use() + ';';
			$translatePartialLoader.addPart(clientwh.contextPath + '/js/common/message/system');
			$translate.refresh();
		}

		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
			console.log('clientwh_system_title');
			$scope.title = $translate.instant('clientwh_system_title');
		});
	...
	$scope.catalog = {};
		...
		$scope.cofirmDeleteToastForm = function(){
			$mdToast.show(
					{  	templateUrl : clientwh.contextPath + '/view/toast.html',
						hideDelay:5000,
						controller  : 'clientwhsystemController',
		...
		$scope.initList = function() {
			// Get permission.
			userService.definePermissionByTarget($scope, 'system').then(
				// success.
				function(response) {
					// List data.
		...
		$scope.showDialogDetail = function () {
	        var htmlUrlTemplate = clientwh.contextPath + '/view/system_view.html';
	    ...
	    $scope.showDialog = function () {
	    	$mdToast.hide();
	    	var htmlUrlTemplate = clientwh.contextPath + '/view/system_form.html';
	    	clientwh.showDialog($scope, $mdDialog, htmlUrlTemplate).then(function(evt) {
	    		console.log('closed');
	    	}, function(evt) {
	    		console.log('not closed');
