...
/**
 * Controller for Origin
 **/

define(['require', 'angular'], function (require, angular) {
	app.aController(clientwh.prefix + 'originController', ['$scope', '$mdToast', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader',clientwh.prefix + 'catalogService', clientwh.prefix + 'userService', 'params',
		...
		if(typeof(clientwh.translate.origin) === 'undefined' || clientwh.translate.origin.indexOf($translate.use()) < 0) {
			console.log(clientwh.translate.origin);
			if(typeof(clientwh.translate.origin) === 'undefined') {
				clientwh.translate.origin = '';
			}
			clientwh.translate.origin += $translate.use() + ';';
			$translatePartialLoader.addPart(clientwh.contextPath + '/js/common/message/origin');
			$translate.refresh();
		}

		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
			console.log('clientwh_origin_title');
			$scope.title = $translate.instant('clientwh_origin_title');
		});
	...
	$scope.catalog = {};
		...
		$scope.cofirmDeleteToastForm = function(){
			$mdToast.show(
					{  	templateUrl : clientwh.contextPath + '/view/toast.html',
						hideDelay:5000,
						controller  : 'clientwhoriginController',
		...
		$scope.initList = function() {
			// Get permission.
			userService.definePermissionByTarget($scope, 'origin').then(
				// success.
				function(response) {
					// List data.
		...
		$scope.showDialogDetail = function () {
	        var htmlUrlTemplate = clientwh.contextPath + '/view/origin_view.html';
	    ...
	    $scope.showDialog = function () {
	    	$mdToast.hide();
	    	var htmlUrlTemplate = clientwh.contextPath + '/view/origin_form.html';
	    	clientwh.showDialog($scope, $mdDialog, htmlUrlTemplate).then(function(evt) {
	    		console.log('closed');
	    	}, function(evt) {
	    		console.log('not closed');
