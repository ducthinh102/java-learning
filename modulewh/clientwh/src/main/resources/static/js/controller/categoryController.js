...
define(['require', 'angular'], function (require, angular) {
	app.aController(clientwh.prefix + 'categoryController', ['$scope', '$mdToast','$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader',clientwh.prefix + 'catalogService', clientwh.prefix + 'userService', 'params',
		...
		if(typeof(clientwh.translate.category) === 'undefined' || clientwh.translate.category.indexOf($translate.use()) < 0) {
			console.log(clientwh.translate.category);
			if(typeof(clientwh.translate.category) === 'undefined') {
				clientwh.translate.category = '';
			}
			clientwh.translate.category += $translate.use() + ';';
			$translatePartialLoader.addPart(clientwh.contextPath + '/js/common/message/category');
			$translate.refresh();
		}

		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
			console.log('clientwh_category_title');
			$scope.title = $translate.instant('clientwh_category_title');
		});
	...
	$scope.catalog = {};
		...
		$scope.initList = function() {
			// Get permission.
			userService.definePermissionByTarget($scope, 'category').then(
				// success.
				function(response) {
					// List data.
		...
		$scope.showDialogDetail = function () {
	        var htmlUrlTemplate = clientwh.contextPath + '/view/category_view.html';
	    ...
	    $scope.showDialog = function () {
	    	$mdToast.hide();
	    	var htmlUrlTemplate = clientwh.contextPath + '/view/category_form.html';
	    	clientwh.showDialog($scope, $mdDialog, htmlUrlTemplate).then(function(evt) {
	    		console.log('closed');
	    	}, function(evt) {
	    		console.log('not closed');
