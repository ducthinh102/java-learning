...
define(['require', 'angular'], function (require, angular) {
	app.aController(clientwh.prefix + 'specController', ['$scope', '$mdToast', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader',clientwh.prefix + 'catalogService',  clientwh.prefix + 'userService', 'params',
		...
		if(typeof(clientwh.translate.spec) === 'undefined' || clientwh.translate.spec.indexOf($translate.use()) < 0) {
			console.log(clientwh.translate.spec);
			if(typeof(clientwh.translate.spec) === 'undefined') {
				clientwh.translate.spec = '';
			}
			clientwh.translate.spec += $translate.use() + ';';
			$translatePartialLoader.addPart(clientwh.contextPath + '/js/common/message/spec');
			$translate.refresh();
		}

		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
			console.log('clientwh_spec_title');
			$scope.title = $translate.instant('clientwh_spec_title');
		});
	...
	$scope.catalog = {};
		...
		$scope.cofirmDeleteToastForm = function(){
			$mdToast.show(
					{  	templateUrl : clientwh.contextPath + '/view/toast.html',
						hideDelay:5000,
						controller  : 'clientwhspecController',
		...
		$scope.initList = function() {
			// Get permission.
			userService.definePermissionByTarget($scope, 'spec').then(
				// success.
				function(response) {
					// List data.
		...
		$scope.showDialogDetail = function () {
	        var htmlUrlTemplate = clientwh.contextPath + '/view/spec_view.html';
	    ...
	    $scope.showDialog = function () {
	    	$mdToast.hide();
	    	var htmlUrlTemplate = clientwh.contextPath + '/view/spec_form.html';
	    	clientwh.showDialog($scope, $mdDialog, htmlUrlTemplate).then(function(evt) {
	    		console.log('closed');
	    	}, function(evt) {
	    		console.log('not closed');
