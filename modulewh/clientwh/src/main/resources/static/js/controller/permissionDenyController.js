...
define(['require', 'angular'], function (require, angular) {
    app.aController(clientwh.prefix + 'permissionDenyController', ['$scope', '$mdToast', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader','$q', clientwh.prefix + 'userService', clientwh.prefix + 'permissionService',
         ...
         // Call service: list all for select.
            ...
            $scope.save = function () {
                ...
                if ($scope.permission.rules!=null) {
                	 ...
                	 tmp.deny = $scope.rules;
                     $scope.permission.rules = JSON.stringify(tmp);
				}
                else {
                	var rule = {}
                	rule.deny = $scope.rules;
                	rule.allow = {};
            ...
            $scope.loadPermission = function (target) {
                ...
                for (var i = 0; i < $scope.listPermission.length; i++) {
                    if ($scope.listPermission[i].target == target) {
                        ...
                        var tmp = JSON.parse($scope.listPermission[i].rules).deny; 
            ...
            $scope.checkConditionAddCreat = function(){
            	$scope.arrTmp = $scope.choseArr(7);
            	if ($scope.checkDenied($scope.choseArr(7))) {
            		 $scope.addOneCreat = false;
            		 return;
				}
            ...
            $scope.checkConditionAddOwner = function(){
            	$scope.arrTmp = $scope.choseArr(12);
            	if ($scope.checkDenied($scope.choseArr(12))) {
            		$scope.addOneOwner = false;
            		 return;
				}
            ...
            $scope.addRuleCreat = function(){
            	...
            	for (var i = 0; i < idSelected.length; i++) {
            	 ...
            	 if (flag==false&&$scope.ruleCreat.createbyids.length!=0) { // Check user denied in createids
            		 if ($scope.forCreate.id > -1) {
            ...
            $scope.cofirmDeleteCreateToastForm = function(id){
    			$mdToast.show(
    					{  	templateUrl : clientwh.contextPath + '/view/toast.html',
    						hideDelay:5000,
    						controller  : 'clientwhpermissionDenyController',
            ...
            $scope.addRuleOwner = function(){
            	...
            	for (var i = 0; i < idSelected.length; i++) {
            	 ...
            	 if (flag==false&&$scope.ruleOwners.ownerbyids.length!=0) { // Check user denided
            		 if ($scope.forOwner.id > -1) {
