...
@Component
public class QuotationdetailSpecificationsBuilder {
 
    public Specification<Quotationdetail> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Quotationdetail> spec1 = null;
		String logic1 = null;
		Specification<Quotationdetail> spec2 = new QuotationdetailSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new QuotationdetailSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new QuotationdetailSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new QuotationdetailSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
