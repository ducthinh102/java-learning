...
@Component
public class MaterialbaselineSpecificationsBuilder {
 
    public Specification<Materialbaseline> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Materialbaseline> spec1 = null;
		String logic1 = null;
		Specification<Materialbaseline> spec2 = new MaterialbaselineSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new MaterialbaselineSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new MaterialbaselineSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new MaterialbaselineSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
