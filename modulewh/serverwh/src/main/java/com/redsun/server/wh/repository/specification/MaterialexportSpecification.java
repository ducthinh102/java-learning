...
public class MaterialexportSpecification implements Specification<Materialexport> {
	...
	public MaterialexportSpecification(SearchCriteria searchCriteria) {
		this.searchCriteria = searchCriteria;
	}

	@Override
	public Predicate toPredicate(Root<Materialexport> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		Predicate result = null;
		query.distinct(true);
		// store.
		if (searchCriteria.getKey().startsWith("store.")) {
			// root.fetch("store", JoinType.INNER);
			Join<Materialexportdetail, Store> store = root.join("store");
