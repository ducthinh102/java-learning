...
@Component
public class MaterialimportdetailSpecificationsBuilder {
 
    public Specification<Materialimportdetail> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Materialimportdetail> spec1 = null;
		String logic1 = null;
		Specification<Materialimportdetail> spec2 = new MaterialimportdetailSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new MaterialimportdetailSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new MaterialimportdetailSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new MaterialimportdetailSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
