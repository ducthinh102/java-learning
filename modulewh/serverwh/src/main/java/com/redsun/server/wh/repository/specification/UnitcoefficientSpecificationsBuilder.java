...
@Component
public class UnitcoefficientSpecificationsBuilder {
 
    public Specification<Unitcoefficient> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Unitcoefficient> spec1 = null;
		String logic1 = null;
		Specification<Unitcoefficient> spec2 = new UnitcoefficientSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new UnitcoefficientSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new UnitcoefficientSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new UnitcoefficientSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
