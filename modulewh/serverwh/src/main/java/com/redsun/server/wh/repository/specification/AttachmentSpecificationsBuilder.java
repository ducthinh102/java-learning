...
@Component
public class AttachmentSpecificationsBuilder {
 
    public Specification<Attachment> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Attachment> spec1 = null;
		String logic1 = null;
		Specification<Attachment> spec2 = new AttachmentSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new AttachmentSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new AttachmentSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new AttachmentSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
