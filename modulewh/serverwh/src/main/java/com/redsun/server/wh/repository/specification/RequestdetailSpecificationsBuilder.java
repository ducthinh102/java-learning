...
@Component
public class RequestdetailSpecificationsBuilder {
 
    public Specification<Requestdetail> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Requestdetail> spec1 = null;
		String logic1 = null;
		Specification<Requestdetail> spec2 = new RequestdetailSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new RequestdetailSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new RequestdetailSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new RequestdetailSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
