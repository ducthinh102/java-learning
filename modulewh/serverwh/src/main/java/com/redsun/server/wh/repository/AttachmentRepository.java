...
public interface AttachmentRepository extends JpaRepository<Attachment, Integer>, JpaSpecificationExecutor<Attachment> {
	...
	@Query("SELECT COUNT(attachment) > 0  FROM Attachment attachment WHERE attachment.id != :id AND attachment.idref = :idref AND attachment.reftype = :reftype AND attachment.filename = :filename AND attachment.status != 3")
	Boolean isExistFileName(@Param("id") Integer id, @Param("idref") Integer idref, @Param("reftype") String reftype, @Param("filename") String filename);
