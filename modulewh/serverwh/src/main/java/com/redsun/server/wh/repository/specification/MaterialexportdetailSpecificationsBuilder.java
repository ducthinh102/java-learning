...
@Component
public class MaterialexportdetailSpecificationsBuilder {
 
    public Specification<Materialexportdetail> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Materialexportdetail> spec1 = null;
		String logic1 = null;
		Specification<Materialexportdetail> spec2 = new MaterialexportdetailSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new MaterialexportdetailSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new MaterialexportdetailSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new MaterialexportdetailSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
