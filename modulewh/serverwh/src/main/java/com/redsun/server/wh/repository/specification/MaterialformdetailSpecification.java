...
public class MaterialformdetailSpecification implements Specification<Materialformdetail> {
	...
	public MaterialformdetailSpecification(SearchCriteria searchCriteria) {
		this.searchCriteria = searchCriteria;
	}

	@Override
    public Predicate toPredicate(Root<Materialformdetail> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		Predicate result = null;
		// material.
		if(searchCriteria.getKey().startsWith("material.")){
			//root.fetch("material", JoinType.INNER);
			Join<Materialformdetail, Material> material = root.join("material");
