...
@Component
public class CatalogSpecificationsBuilder {
 
    public Specification<Catalog> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Catalog> spec1 = null;
		String logic1 = null;
		Specification<Catalog> spec2 = new CatalogSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new CatalogSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new CatalogSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new CatalogSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
