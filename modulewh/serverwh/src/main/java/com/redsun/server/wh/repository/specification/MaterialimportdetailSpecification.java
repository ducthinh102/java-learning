...
public class MaterialimportdetailSpecification implements Specification<Materialimportdetail> {
	...
	public MaterialimportdetailSpecification(SearchCriteria searchCriteria) {
		this.searchCriteria = searchCriteria;
	}

	@Override
    public Predicate toPredicate(Root<Materialimportdetail> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if(searchCriteria.getKey().startsWith("material.")){
			...
			Join<Materialimportdetail, Material> material = root.join("material");
