...
public class QuotationdetailSpecification implements Specification<Quotationdetail> {
	...
	public QuotationdetailSpecification(SearchCriteria searchCriteria) {
		this.searchCriteria = searchCriteria;
	}

	@Override
    public Predicate toPredicate(Root<Quotationdetail> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if(searchCriteria.getKey().startsWith("material.")){
			...
			Join<Requestdetail, Material> material = root.join("material");
