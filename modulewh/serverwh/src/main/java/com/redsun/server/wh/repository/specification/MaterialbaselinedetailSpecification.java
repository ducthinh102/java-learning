...
public class MaterialbaselinedetailSpecification implements Specification<Materialbaselinedetail> {
	...
	public MaterialbaselinedetailSpecification(SearchCriteria searchCriteria) {
		this.searchCriteria = searchCriteria;
	}

	@Override
    public Predicate toPredicate(Root<Materialbaselinedetail> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		Predicate result = null;
		// material.
				if(searchCriteria.getKey().startsWith("material.")){
					//root.fetch("material", JoinType.INNER);
					Join<Materialbaselinedetail, Material> material = root.join("material");
