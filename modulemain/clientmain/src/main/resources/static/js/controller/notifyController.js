...
define(['require', 'angular'], function (require, angular) {
	app.aController(clientmain.prefix + 'notifyController', ['$scope', '$state', '$rootScope', '$mdDialog', '$http', '$log', '$window', '$location', '$filter', '$translate', '$translatePartialLoader', clientmain.prefix + 'notifyService',
		function($scope, $state, $rootScope, $mdDialog, $http, $log, $window, $location, $filter, $translate, $translatePartialLoader, notifyService) {
		if(typeof(clientmain.translate.notify) === 'undefined' || clientmain.translate.notify.indexOf($translate.use()) < 0) {
			if(typeof(clientmain.translate.notify) === 'undefined') {
				clientmain.translate.notify = '';
			}
			clientmain.translate.notify += $translate.use() + ';';
			$translatePartialLoader.addPart(clientmain.contextPath + '/js/common/message/notify');
			$translate.refresh();
		}
		
		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
		    $scope.title = $translate.instant('clientmain_notify_title');
		});
		// Unregister
	    ...
	    $scope.showDialog = function () {
	        var htmlUrlTemplate = clientmain.contextPath + '/view/notify_form.html';
