...
public class UsersSpecificationsBuilder {
    ...
    public Specification<Users> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Users> spec1 = null;
		String logic1 = null;
		Specification<Users> spec2 = new UsersSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new UsersSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new UsersSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new UsersSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
