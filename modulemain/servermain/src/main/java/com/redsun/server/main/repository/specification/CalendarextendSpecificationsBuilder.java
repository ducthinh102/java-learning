...
@Component
public class CalendarextendSpecificationsBuilder {
 
    public Specification<Calendarextend> build(List<SearchCriteria> searchCriterias) {
        if (searchCriterias.size() == 0) {
			return null;
		}
		SearchCriteria searchCriteria = searchCriterias.get(0);
		Specification<Calendarextend> spec1 = null;
		String logic1 = null;
		Specification<Calendarextend> spec2 = new CalendarextendSpecification(searchCriteria);
		...
		for (int i = 1; i < size; i++) {
			...
			if(searchCriteria.getKey().equals("")) {
				...
				i++;
				searchCriteria = searchCriterias.get(i);
				spec2 = new CalendarextendSpecification(searchCriteria);
				logic2 = searchCriteria.getLogic();
			} else {
				if (logic2 != null && logic2.toUpperCase().equals("AND")) {
					spec2 = Specifications.where(spec2).and(new CalendarextendSpecification(searchCriteria));
				}
				else {
					spec2 = Specifications.where(spec2).or(new CalendarextendSpecification(searchCriteria));
				}
				logic2 = searchCriteria.getLogic();
			}
		}
